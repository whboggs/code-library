...
| `DLV - content_category` | `content_category` | String | Category of the page/product. Optional. | `items[].item_category` |
| `DLV - content_ids` | `content_ids` | Array of integers or strings | Product IDs associated with the event, such as SKUs. | `items[].item_id` |
| `DLV - content_name` | `content_name` | String | Name of the page/product. Optional. | `items[].item_name` |
| `DLV - content_type` | `content_type` | String | Either `product` or `product_group` based on the IDs passed in `content_ids` or `contents`. | — (no GA4 equivalent) |
| `DLV - contents` | `contents` | Array of objects | Array of JSON objects with product IDs and quantities. `id` and `quantity` required. | `items[]` array |
